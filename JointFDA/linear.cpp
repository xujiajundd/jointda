...
void solve_l2r_lr_dual(const problem *prob, float *w, float eps, float Cp, float Cn)
{
    ...
    float innereps = 1e-2;
    float innereps_min = min(1e-8, (double)eps);
    ...
    while (iter < max_iter)
    {
        ...
        if(newton_iter <= l/10)
            innereps = max((double)innereps_min, 0.1*innereps);
...
//    }
	...
	if (SAVEBIN)
	{
		FILE* file;
		file=fopen("allW","ab+");
		fwrite(model_->w,sizeof(float),w_size*nr_w,file);
		fclose(file);
	}
